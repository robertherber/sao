...
const doInstall = (dependencies, defaultOptions, customOpts = {}) => {
  const depsIsOptions =
    typeof dependencies === 'object' && !Array.isArray(dependencies)

  const options = depsIsOptions ? dependencies : customOpts

  if (!dependencies || depsIsOptions) {
    npmInstall({ ...defaultOptions, ...options })
  } else {
    npmInstall(dependencies, { ...defaultOptions, ...options })
  }
}
